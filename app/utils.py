...
def get_fernet():
    ...
    if not key:
        # For development, generate a key if not present (NOT RECOMMENDED FOR PRODUCTION)
        # Ideally, we should flash a warning or error.
        key = Fernet.generate_key().decode()
        print(f"WARNING: FERNET_KEY not found in environment. Using temporary key: {key}")
