    ...
    for c in campaigns:
        for r in c.recipients:
            data.append({
                ...
                'Clicks': sum(1 for e in r.events if e.type == 'click'),
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        if not campaign_id:
            ...
            if not summary_df.empty:
                cols_to_keep = ['name', 'subject', 'status', 'created_at', 'total_recipients', 'sent_count', 'open_count', 'click_count']
                summary_df = summary_df[cols_to_keep]
                summary_df.columns = ['Campaign', 'Subject', 'Status', 'Date', 'Recipients', 'Sent', 'Opens', 'Clicks']
